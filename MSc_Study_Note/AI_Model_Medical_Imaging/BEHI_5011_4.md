...
- 为达到不互斥，通过**堆叠**独立的每个标签的二分类Binary Classification即可实现多标签分类。
...
1. 迁徙学习真的不会造成模型范式污染么？
   ...
   Does transfer learning really wouldn’t cause model paradigm pollution？

   **基本不会，而且提升会很大**。

2. 为什么用的是迁徙学习而不是强化学习，迁徙学习+强化学习是否能用于需要长期跟踪的病例中，譬如营养状态与需求？

   Why is transfer learning used in medical image learning instead of reinforcement learning?
...
<img src="./BEHI_5011_4.assets/49.png" alt="two-stage cascaded framework" style="zoom:50%;" />

1. 核心方法

基于3D CNN提出**two-stage cascaded framework**，两阶段级联框架，既利用3D空间信息区分“真实病灶vs相似物”，又保证检测效率。

2. 两阶段流程

> Stage 1：**3D FCN粗筛（Screening）**

- 输入：完整3D脑MRI容积数据（Testing volume）
- 作用：快速扫描全脑，生成**3D score volume**，高亮**所有潜在候选病灶区域**（遵循“宁错勿漏”原则）
- 目的：大幅缩小后续精细检测的搜索范围，提升整体效率。

> Stage 2：**3D CNN精判别（Discrimination）**

- 输入：stage 1的candidates
- 作用：对每个候选区域提取**3D 立体特征**，精准区分 “真实微出血” 和 “相似物 / 噪声”，输出最终检测结果（Detection result，绿色框为正确检测的病灶）。
- 目的：利用 3D 空间信息（连续切片的层间关联、立体形态）解决 “易混淆” 问题，提升检测精度。

3. 训练策略（Training strategy）

- **样本选择sample selection**：构建三类训练样本，精准应对“易混淆模仿物”的挑战：
  - **Positive samples**：正样本，真实脑微出血灶的3D块；
  - **Randomly negative samples**：大脑中随机选取的正常区域；
  - **False positive samples**：模型初步误判为病灶的区域（即“模仿物”，如血管断面、伪影）
- **微调fine-tuning**的流程：
  1. 先在**正负样本**上训练传统3D CNN；
  2. 再加入**假阳性样本**构建新训练集；
  3. 对模型进行fine-tuning，让模型重点学习“过滤假阳性”的能力。

4. 测试策略（Testing strategy）

- **Fully convolutional variant，全卷积变体**：将传统 3D CNN 的**全连接层（FC）替换为 1×1×1 卷积层**，改造为 3D 全卷积网络（3D FCN）：
  - 优势：可直接处理**任意大小的 3D 测试容积**，无需分块裁剪，一次前向传播即可输出全脑的 3D 得分体（3D score volume），提升检测效率。
- **Sparse location mapping，稀疏位置映射**：将 3D 得分体中的**高分候选区域**，映射回原始 3D MRI 的空间坐标，得到最终的病灶检测结果，精准定位稀疏分布的微小病灶。

5. 样本预处理（Sample preprocessing）：

- **Intensity normalization，强度归一化**：消除不同被试、不同扫描设备间的灰度差异，让模型专注学习病灶的**形态 / 空间特征**，而非灰度偏差。
- **Training samples augmentation，训练样本增强**：通过**旋转、平移**等几何变换扩充训练集，增加数据多样性，缓解 “病灶稀疏、样本量有限” 导致的过拟合问题。

6. 评估指标：Sensitivity、Precision、FP per subject ($FP_{avg}$)

   $FP_{avg}$：每个被试的平均假阳性数 → 量化模型误判 “模仿物” 的程度，控制假阳性在临床可接受范围内。
...
**模型同时适用多个不同但相关任务的数据进行训练**，而不是孤立地训练每个任务。

> 例如：在胸片 AI 中，同时训练「肺炎检测」「结节检测」「气胸识别」等多个任务，而不是为每个疾病单独训练一个模型。

2. 核心优势

- **Complementary knowledge mining（补充知识挖掘）**：不同任务之间的知识可以互补。

> 比如肺炎和结节的特征有重叠，同时学习能让模型捕捉到更全面的胸部影像特征，提升各自任务的性能。

- **Transfer learning（迁移学习）**：利用任务间的相关性，把一个任务学到的知识迁移到另一个任务。尤其适合小样本医疗任务。

> 比如借助「肺炎检测」的大数据，提升罕见病「结节分类」的性能。

- **Robust（更鲁棒）**：多任务学习让模型学习到更通用的特征，减少过拟合，对噪声和分布变化更不敏感，在临床真实数据中更稳定
...
**Shared feature extractor，共享特征提取器**：是多任务学习最核心、最常用的架构，核心思想是多个任务共享底层的特征提取模块（如CNN卷积层、Transformer编码器），充分利用任务间的共性特征，减少参数、提升泛化能力。

![Common architectures](./BEHI_5011_4.assets/50.png)

> 下半部分架构图中，`shared features`直接并行输入到`task1`、`task2`、`task3`，每个任务独立输出结果。

- **Separate heads，独立输出头**：每个任务拥有独立的输出层（head），任务之间并行处理、无依赖关系。

<img src="./BEHI_5011_4.assets/51.png" alt="cascaded head" style="zoom:70%;" />

> 半部分架构图中，`task 1` 的输出传递给 `task 2`，`task 2` 的输出再传递给 `task 3`，任务按顺序执行。

- **Cascaded heads，级联输出头**：任务间存在依赖关系，前一个任务的输出作为后一个任务的输入，形成 “链式” 级联。



##### Separate feature extractor

**Separate feature extractor，独立特征提取器**：通过Cross-stitch单元让两个独立的任务网络在特征层面进行信息交互，兼顾“任务特异性”与“知识共享”：

<img src="./BEHI_5011_4.assets/52.png" alt="separate feature extractor" style="zoom:75%;" />

- 上方橙色的`Network A`：为`task A`专属的特征提取器；
- 下方绿色的`Network B`：为`task B`专属的特征提取器；

两者格子拥有独立的卷积、池化和全连接层，提取任务专属的特征，避免强制共享底层特征导致任务特异性丢失。



- **Cross-stitch单元**（$\alpha$模板）：

在网络的多个特征层之间插入Cross-stitch单元，通过可学习的权重$\alpha$，对两个网络的对应层特征进行**线性混合与信息传递**：

$$Mix\ feature=\alpha \times Feature \ of \ A +(1-\alpha)\times Feature\ of\ B$$
...
#### Case study: Lung Nodule Analysis
