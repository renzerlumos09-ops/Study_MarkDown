...
- **参数更新公式**：$g=\frac{1}{m^′}∇_θ\sum\limits_{i=1}^{m^′}L(x^{(i)},y^{(i)},θ)$
...
何为CNN？**Convolution Neural Network**，是DL解决Fully connect处理图像时效率低下的Key core. 组成如下：
...
**RNN（Recurrent Neural Networks）**，通过递归定义的动态系统——时刻$t$的状态$s^{(t)}$，仅由前一个时刻$t-1$的状态$s^{(t-1)}$和参数$θ$决定。
