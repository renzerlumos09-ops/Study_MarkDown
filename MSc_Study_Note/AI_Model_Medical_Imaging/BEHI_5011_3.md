...
机器学习（ML）是一个计算机科学领域，它使计算机能够在**不明确编程**的情况下进行学习。
...
> 为啥前面有负号？
>
> $f(x)∈(0,1)$，因此想要loss function为正，则需要加上负号
...
> $f_* = \arg \min\limits_{f∈\mathcal{F}}R(f)$
...
>
> $\hat{R}(f, d) = \frac{1}{N} \sum\limits_{(x_i, y_i)∈d}\mathcal{L}(y_i,f(x_i))$​
...
> $f^d_* = \arg \min\limits_{f∈\mathcal{F}}\hat{R}(f, d)$

- $f^d_*$：在训练集 $d$ 上，$\hat{R}_{min}(f, d)$找到的“最优模型”。
- $\arg \min\limits_{f∈\mathcal{F}}$：$\mathcal{F}$ 中，能使$\hat{R}_{min}(f, d)$的函数 $f$。
...
- Generalization 泛化能力，上述**[Q&A]**已解答
...
<img src="./BEHI_5011_3.assets/VS-1770635817019-1.png" alt="DL" style="zoom:67%;" />

### 最Old School的Neural network

机器学习和人工智能的老资历Neural network神经网络最早可追溯到20世纪40年代（1940s）。
...
<img src="./BEHI_5011_3.assets/47.png" alt="Old school" style="zoom:45%;" />

> At first, we don’t know the bias and weight of the best perform function.
...
<img src="./BEHI_5011_3.assets/48.png" alt="compare" style="zoom:45%;" />
...
- **Max-pooling**：最大池化，在局部区域取最大值，能更好保留边缘、纹理等显著特征。(**<u>最常用的池化</u>**，我们认为保留最大的特征能代表数据)
- **Average pooling**：在局部区域取平均值，能保留区与的整体强度信息。
- **$L_2$ pooling**：在局部区域计算$L_2$范数，能平衡最大值和平均值的效果，提升鲁棒性。
...
Pay attention on _From signal to noise_.
...
1. ML或DL想要解决SL或USL的目标，那么就需要一个**标杆/标准**来定义程序范式的好坏（鲁棒性、泛化能力等指标的优劣），因此我们在这里使用**“损失函数”（Loss function）**来量化标准。
...
5. 因此，ERM也就是最小化训练集的经验风险，便是Model所追求的目标，它代表着Model所使用的算法在假设空间中最接近的、最具鲁棒性、泛化能力最强的算法。
